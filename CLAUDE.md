...
- 不要常驻侧边栏（仅当打开文件时显示同目录文件列表，可 ⌘⇧B 隐藏）
- 不要状态栏
- 界面只有：标题栏（拖拽用）+ 编辑器（+ 打开文件时的同目录文件列表）
- 追求极致的简单，一个功能做到极致
...
3. **同目录文件列表** — 打开文件时显示同目录的 Markdown 文件，点击即切换，Agent 新建/删除文件实时刷新
4. **主题系统** — CSS 主题，可导入自定义主题
5. **导出** — PDF

### 不做的事情

- 不做跨目录文件树、工作区（只支持打开文件所在目录的 Markdown 文件列表）
- 不做知识库管理
- 不做云同步、协作编辑
- 不做笔记组织和标签系统
